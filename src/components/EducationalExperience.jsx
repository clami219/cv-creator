    ...
    if(editMode) {
        return (
            <div className="flex flex-col text-left gap-4 mt-0 p-6 pt-2">
                <Input title="Title of study" value={exp.titleOfStudy} onChange={(e)=>{expChange(exp.id,e.target.name, e.target.value)}} name="titleOfStudy" mandatory={mandatoryFields.indexOf("titleOfStudy") !== -1}></Input>
                <Input title="School name" value={exp.schoolName} onChange={(e)=>{expChange(exp.id,e.target.name, e.target.value)}}  name="schoolName" mandatory={mandatoryFields.indexOf("schoolName") !== -1}></Input>
...
function validate(exp)
{
    ...
    if(!feedback)
        alert("Attenzione! E' necessario compilare tutti i campi!");
