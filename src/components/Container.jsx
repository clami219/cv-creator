    ...
    return (
        <div className="group flex flex-col md:w-180 lg:w-250 gap-2 mt-6 border border-gray-300 border-dotted rounded-lg p-6 pt-2">
            <div className="flex items-center justify-between mt-4 mb-0">
                ...
                <div className={`flex items-center gap-4 hidden ${change.visible ? 'group-hover:block' : 'group-hover:hidden'}`}>
                    <PlusIcon className="h-6 w-6 text-gray-500 hover:text-indigo-500 cursor-pointer" onClick={change.onClick}/>
                </div>
            </div>
            {children}
        </div>
